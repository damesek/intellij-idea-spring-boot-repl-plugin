...
**Current version: 0.20.0 · IntelliJ IDEA through 2025.2.x · Java 17/21 tested · 60 MCP tools**

[Hungarian PDF handbook](output/pdf/spring-boot-repl-guide-hu.pdf) · [Claude setup](docs/claude-repl-guide-hu.md) · [AI agent instructions and tool reference](docs/claude-repl-instructions.md) · [Protocol](PROTOCOL.md)
...
| Recorded call graph, source values, filters, timeline and reference comparison | **Tap / Trace → Recorded calls** |
...
Version 0.20 adds **11 MCP recording tools**: agents can read captured inputs, results, source and timelines, compare calls, and—with the corresponding permissions—select a node in IDEA or start/stop recording. See the [implementation and validation report](MCP_RECORDINGS_0_20.md).

## Quick start

1. With JDK 17 installed, run `./gradlew :buildPlugin`. This includes regression checks and produces `build/distributions/sb-repl-0.20.0.zip`.
...
Selecting a node opens its source with the captured input and result. If the current source differs, a captured source copy remains available. Recording observes future calls in 1–8 selected classes, with a maximum of 200 calls and a 32 MiB preview budget. Cross-thread async/reactive relationships are not inferred. Values are bounded display evidence, not a saved JVM stack or restorable live object.
...
Add the optional matching [0.20.0 bridge](sb-repl-bridge/README.md) to your development application:
...
The **54-page Hungarian handbook** covers all **11 main tabs**, their buttons/fields, keyboard actions and all **60 MCP tools**:

- [Read/download the PDF](output/pdf/spring-boot-repl-guide-hu.pdf).
- Open the bundled copy through **Code → Spring Boot REPL → Help (PDF)**, the editor context menu or workbook Help, even while disconnected.
- [Read the handbook source](docs/repl-help-hu.md), [Claude setup](docs/claude-repl-guide-hu.md) or [protocol reference](PROTOCOL.md).

Rebuild the PDF with `python3 scripts/build-help-pdf.py` using ReportLab and DejaVu Sans/Mono or Arial/Courier New fonts. Normal plugin builds use the checked-in PDF and do not require Python.
...
The [0.20 validation](MCP_RECORDINGS_0_20.md) records **295 passing tests on each JDK** and **Compatible** results on both 2025.2 editions. Tests cover JShell/Spring integration, transaction rollback, snapshots, debugger capture, recordings, UI models and MCP permissions. The verifier still reports deprecated/scheduled-removal API usages. These results do not imply verification against IDEA versions after 2025.2 or every application's runtime configuration.
